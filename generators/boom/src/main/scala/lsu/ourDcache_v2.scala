...
/** DOUGLAS:
  * This module encapsulate the metadata arrays and data arrays, as well as the prediction table
  */
class DCacheArrays (implicit p: Parameters) extends BoomBundle()(p)
    with HasL1HellaCacheParameters
    with HasBoomCoreParameters
{
    val io = new Bundle {
        val meta_read   = Decoupled(new BoomL1MetaReadReq).flip
        val meta_write  = Decoupled(new L1MetaWriteReq).flip
        val data_read   = Decoupled(new BoomL1DataReadReq).flip
        val data_write  = Decoupled(new L1DataWriteReq).flip
        val data_resp   = Output(Vec(memWidth, Vec(nWays, Bits(encRowBits.W))))
        val meta_resp   = Output(Vec(nWays, new L1Metadata))
        val s0_req      = Input(Vec(memWidth, new BoomDCacheReq)) // we need this for the whole address. Addresses in meta req and data req
    ...
    for (w <- 0 until memWidth) {
        meta_spatial(w).io.read.valid  := meta_read.valid
        meta_spatial(w).io.read.bits   := meta_read.bits.req(w)

        meta_temporal(w).io.read.valid  := meta_read.valid
        meta_temporal(w).io.read.bits   := meta_read.bits.req(w)
    }

    // meta write TODO:
    for (w <- 0 until memWidth) {
        meta_spatial(w).io.write.valid := meta_write.fire()
        meta_spatial(w).io.write.bits  := meta_write.bits

        meta_temporal(w).io.write.valid := meta_write.fire()
        meta_temporal(w).io.write.bits  := meta_write.bits
    }

    // meta ready
    meta_read.ready  := meta_spatial.map(_.io.read.ready).reduce(_||_) && meta_temporal.map(_.io.read.ready).reduce(_||_)
    meta_write.ready := meta_spatial.map(_.io.write.ready).reduce(_||_) && meta_temporal.map(_.io.write.ready).reduce(_||_)


    // data read
    for (w <- 0 until memWidth) {
        data_spatial.io.read(w).valid := data_read.bits.valid(w) && data_read.valid
        data_spatial.io.read(w).bits  := data_read.bits.req(w)

        data_temporal.io.read(w).valid := data_read.bits.valid(w) && data_read.valid
        data_temporal.io.read(w).bits  := data_read.bits.req(w)
    }
    data_read.ready := true.B

    // data write TODO:
    data.io.write.valid := data_write.fire()
    data.io.write.bits  := data_write.bits
    data_write.ready    := true.B

    // hit check. For temporal cache we need also check the word index
    val s0_addr = widthMap(i => s0_req(i).addr)
    val spatial_hit = widthMap(i => wayMap((w: Int) => meta_spatial(i).io.resp(w).tag === (s0_addr(i) >> untagBits)).asUInt)
    val temporal_hit = widthMap(i => wayMap((w: Int) => meta_temporal(i).io.resp(w).tag === (s0_addr(i) >> untagBits) &&
                                                        meta_temporal(i).io.resp(w).wordIdx === s0_addr(i)(offsetmsb, offsetlsb))) // get word index
