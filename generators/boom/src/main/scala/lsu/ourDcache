...
class BoomDuplicatedDataArray_temporal(implicit p: Parameters) extends AbstractBoomDataArray
{

  val waddr = io.write.bits.addr >> rowOffBits
  for (j <- 0 until memWidth) {  // DOUGLAS: memWidth is 1
  ...
  with HasBoomCoreParameters
{
  ...
  for (w <- 0 until memWidth) {
    ...
    // TODO: choose which metadata array to store the new cacheline
    meta_spatial(w).io.write.valid := metaWriteArb.io.out.fire() // meta write in mshrs.scala
    meta_spatial(w).io.write.bits  := metaWriteArb.io.out.bits
    meta_spatial(w).io.read.valid  := metaReadArb.io.out.valid
    meta_spatial(w).io.read.bits   := metaReadArb.io.out.bits.req(w)

    meta_temporal(w).io.write.valid := metaWriteArb.io.out.fire()
    meta_temporal(w).io.write.bits  := metaWriteArb.io.out.bits
    meta_temporal(w).io.read.valid  := metaReadArb.io.out.valid
    meta_temporal(w).io.read.bits   := metaReadArb.io.out.bits.req(w)
  }
  ...
  /** DOUGLAS:
    * metadata array ready signal generation. TODO: Not sure if correct
    */
  metaReadArb.io.out.ready  := meta_spatial.map(_.io.read.ready).reduce(_||_) && meta_temporal.map(_.io.read.ready).reduce(_||_)
  metaWriteArb.io.out.ready := meta_spatial.map(_.io.write.ready).reduce(_||_) && meta_temporal.map(_.io.read.ready).reduce(_||_)
  ...
  for (w <- 0 until memWidth) {
    ...
    /** DOUGLAS:
     *  data read commands. We access two data arrays simultaneously
     */
    data_spatial.io.read(w).valid := dataReadArb.io.out.bits.valid(w) && dataReadArb.io.out.valid
    data_spatial.io.read(w).bits  := dataReadArb.io.out.bits.req(w)
    data_temporal.io.read(w).valid := dataReadArb.io.out.bits.valid(w) && dataReadArb.io.out.valid
    data_temporal.io.read(w).bits  := dataReadArb.io.out.bits.req(w)
  }
  ...
  /** DOUGLAS:
   *  data write commands. We need to choose where the data goes
   */
  val s3_spatial_hit = Wire(Vec(memWidth, Bool()))
  data_spatial.io.write.valid := dataWriteArb.io.out.fire() && s3_spatial_hit(0)
  data_spatial.io.write.bits  := dataWriteArb.io.out.bits
  data_temporal.io.write.valid := dataWriteArb.io.out.fire() && !s3_spatial_hit(0)
  data_temporal.io.write.bits  := dataWriteArb.io.out.bits

  dataWriteArb.io.out.ready := true.B

  // ------------
  // New requests
  // DOUGLAS: Important logic here!
  ...
  // DOUGLAS: choose which request to fire
  ...
  val s1_tag_eq_way_spatial  = widthMap(i => wayMap((w: Int) => meta_spatial(i).io.resp(w).tag === (s1_addr(i) >> untagBits)).asUInt)
  val s1_tag_eq_way_temporal = widthMap(i => wayMap((w: Int) => meta_temporal(i).io.resp(w).tag === (s1_addr(i) >> untagBits) &&
                                                                meta_temporal(i).io.resp(w).wordIdx === s1_addr(i)(offsetmsb, offsetlsb)).asUInt)
  ...
  val s1_tag_match_way_spatial = widthMap(i =>
                         Mux(s1_type === t_replay, s1_replay_way_en,
                         Mux(s1_type === t_wb,     s1_wb_way_en,
                         Mux(s1_type === t_mshr_meta_read, s1_mshr_meta_read_way_en,
                           wayMap((w: Int) => s1_tag_eq_way_spatial(i)(w) && meta_spatial(i).io.resp(w).coh.isValid()).asUInt))))
  val s1_tag_match_way_temporal = widthMap(i =>
                         Mux(s1_type === t_replay, s1_replay_way_en,
                         Mux(s1_type === t_wb,     s1_wb_way_en,
                         Mux(s1_type === t_mshr_meta_read, s1_mshr_meta_read_way_en,
                           wayMap((w: Int) => s1_tag_eq_way_temporal(i)(w) && meta_temporal(i).io.resp(w).coh.isValid()).asUInt))))
  ...
  // val s2_tag_match_way = RegNext(s1_tag_match_way) // DOUGLAS: S2 means pipeline stage 2
  val s2_tag_match_way_spatial = RegNext(s1_tag_match_way_spatial) // (memWidth, nWays)
  val s2_tag_match_way_temporal = RegNext(s1_tag_match_way_temporal)

  // val s2_tag_match     = s2_tag_match_way.map(_.orR)
  val s2_spatial_match = s2_tag_match_way_spatial.map(_.orR) // (memWidth,)
  val s2_temporal_match = s2_tag_match_way_temporal.map(_.orR)
  val s2_tag_match    = widthMap(i => s2_spatial_match(i) || s2_temporal_match(i))
  ...
  // for (i <- 0 until memWidth) {
  //   for (w <- 0 until nWays) {
  //     s2_data(i)(w) := data.io.resp(i)(w) // DOUGLAS: Output from data array
  //   }
  // }
  val s2_data_spatial = Wire(Vec(memWidth, Vec(nWays, UInt(encRowBits.W))))
  val s2_data_temporal = Wire(Vec(memWidth, Vec(nWays, UInt(encRowBits.W))))
  for (i <- 0 until memWidth) {
    for (w <- 0 until nWays) {
      s2_data_spatial(i)(w) := data_spatial.io.resp(i)(w)
    }
  }
  for (i <- 0 until memWidth) {
    for (w <- 0 until nWays) {
      s2_data_temporal(i)(w) := data_temporal.io.resp(i)(w)
    }
  }


  /** DOUGLAS:
    * Get the right data based on which cache is hit
    */
  // val s2_data_muxed = widthMap(w => Mux1H(s2_tag_match_way(w), s2_data(w)))
  val s2_data_muxed = widthMap(w => Mux(s2_spatial_match(w), Mux1H(s2_tag_match_way_spatial(w), s2_data_spatial(w)), 
                                                             Mux1H(s2_tag_match_way_temporal(w), s2_data_temporal(w))))
  ...
  // hits always send a response <= DOUGLAS: Important info here!
  ...
  // Miss handling     DOUGLAS: miss handling logic here
  for (w <- 0 until memWidth) {
  ...
  val resp = WireInit(cache_resp) // DOUGLAS: this variable stores the response data when hit
  ...
  /** DOUGLAS:
    * We need the word index if we are writing to temporal cache.
    * We also need to know which cache we are writing to through hit info
    */
  val s3_word_idx = RegNext(s2_word_idx)
  val s3_spatial_hit = RegNext(s2_spatial_match)
  ...
  dataWriteArb.io.in(0).bits.data   := Fill(rowWords, s3_req.data) // DOUGLAS: Fill replicates a value multiple times
