    ...
    with HasL1HellaCacheParameters
{
    ...
    * **************/
    ...
    for (i <- 0 until memWidth) {
        data_spatial.io.read(i).valid := io.data_read.bits.valid(i) && io.data_read.valid
        data_spatial.io.read(i).bits  := io.data_read.bits.req(i)

        data_temporal.io.read(i).valid := io.data_read.bits.valid(i) && io.data_read.valid
        data_temporal.io.read(i).bits  := io.data_read.bits.req(i)
    }
    ...
    * **************/
    ...
    val s0_addr = io.s0_req(0).addr // assume memWidth is 1
...
//   metaWriteArb.io.out.ready := meta.map(_.io.write.ready).reduce(_||_)
   ...
   */
  ourCache.io.meta_write <> metaWriteArb.io.out
...
//   dataWriteArb.io.out.ready := true.B
   ...
   */
  ourCache.io.data_read <> dataReadArb.io.out
  ourCache.io.data_write <> dataWriteArb.io.out
  ...
  // tag check
  def wayMap[T <: Data](f: Int => T) = VecInit((0 until nWays).map(f))
  val s1_tag_eq_way = widthMap(i => wayMap((w: Int) => ourCache.io.meta_resp(i)(w).tag === (s1_addr(i) >> untagBits)).asUInt)
  val s1_tag_match_way = widthMap(i =>
                         ...
                         Mux(s1_type === t_mshr_meta_read, s1_mshr_meta_read_way_en,
                           wayMap((w: Int) => s1_tag_eq_way(i)(w) && ourCache.io.meta_resp(i)(w).coh.isValid()).asUInt)))) // DOUGLAS: connect metadata response
...
//   val s2_nack_data   = widthMap(w => data.io.nacks(w))
  ...
  mshrs.io.meta_resp.bits       := Mux1H(s2_tag_match_way(0), RegNext(ourCache.io.meta_resp(0)))
