  ...
  extends IssueUnit(params.numEntries, params.issueWidth, numWakeupPorts, params.iqType, params.dispatchWidth)
{
  ...
  for (idx <- 0 until numIssueSlots) {
    
    // Ensure that similar amount of old and young instructions are issued
    var i = if (idx%2 == 0) idx/2 else (numIssueSlots/2)+(idx/2)
