...
void multigather_smallbuf_serial(
    double* restrict target,
    const double* restrict source, // source array
    const int* restrict inner_pat, // ininnerattern array
    const int* restrict outer_pat, // outer pattern array
    int pat_len,  // length of inner pattern
    int delta, // stride between each gather
    int n, // number of gathers
    int target_len) {
...
int main(int argc, char* argv[]) {
       ...
       setStats(0);
        ...
        double target[target_len * pat_len]; 
        setStats(1);
        multigather_smallbuf_serial(target, source, inner_pat, outer_pat, pat_len, delta, n, target_len);
        // Print the resulting matrix
        // for (int i = 0; i < target_len; i++) {
        //     for (int j = 0; j < pat_len; j++) {
        //         // Accessing element in row-major order
